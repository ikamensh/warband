...
def test_a_saved_match_gives_back_its_tallies():
    ...
    back = from_record(json.loads(json.dumps(to_record(played))))
    # The whole result is not compared: its styles hold NaN for "never attacked", and NaN is not equal to itself.
    assert (back.spec, back.placements, back.settled, back.tallies) == \
           (played.spec, played.placements, played.settled, played.tallies)
