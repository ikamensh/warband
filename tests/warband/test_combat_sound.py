"""Combat impacts are playable and keep their physical contrasts: the Foley cut from generated pieces, and the one
synthesised family beside it, a healer's mote of light."""

import numpy as np

from sagaforge import synth
from warband.audio import combat_sound, sound

#: Every weapon the bank has impacts for, whoever makes them: what holds for the Foley holds for the mote.
WEAPONS = sorted({impact.rsplit("_", 1)[0] for impact in sound.IMPACTS})
...
def test_hard_armor_rings_brighter_than_flesh_and_siege_stones_land_an_octave_lower_than_arrows() -> None:
    ...
    for variant in range(combat_sound.VARIANTS):
        for weapon in WEAPONS:
            armor = sound.SOUNDS[f"{weapon}_armor_{variant}"]()
            flesh = sound.SOUNDS[f"{weapon}_flesh_{variant}"]()
...
def test_takes_are_repeatable_but_do_not_reuse_the_same_waveform() -> None:
    ...
    for weapon in WEAPONS:
        for material in combat_sound.MATERIALS:
            takes = [sound.SOUNDS[f"{weapon}_{material}_{variant}"]()
                     for variant in range(combat_sound.VARIANTS)]
            for variant, samples in enumerate(takes):
                assert np.array_equal(samples, sound.SOUNDS[f"{weapon}_{material}_{variant}"]())
