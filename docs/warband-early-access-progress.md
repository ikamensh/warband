...
| W10 stability, performance | **evidence in place** (300-match run and soak in progress) | Fuzz across the increments: 120 AI matches and 182 monkey runs; three movement deadlocks found and fixed with regression tests (head-on separation `feb8046`; corner steering `bbdbd53`; a walk held short of its spot by a pinned crowd `7c7f3cd`, found by the 300-match run). Monkey: `tools/fuzz_warband.py --games 0 --monkey 100 --steps 1000 --seed 1000` → 100 played, 0 failed. Frame times from `tools/perf_warband.py` (150 units of six types between twelve farms, real backend, images pre-rendered as after the opening's warm-up): `setup frame 195 ms; 720 frames: p50 5.9 ms, p95 13.1 ms, max 30.7 ms; last 120 frames: p50 4.6 ms, p95 11.6 ms` on the reference Mac (M-series, 2026-09-05); with two other CPU-bound processes running, p95 rises to 17 ms. The gate is met on an idle reference Mac. Cuts that got there: GL error checking off, pooled `draw_image` sprites, appearance setters that skip unchanged values, `Y_SORT_STEP` groups, grid A*, direct steering, cached panel geometry. The earlier 126 ms p95 figure was measured under `tracemalloc`, which the tool avoids. Pending: the 300-match fuzz on the fixed code and the 30-minute soak (`tools/soak_warband.py`), both queued. |
| W11 tests | ongoing | 528 tests green at `d3be67f` (Warband 114); CI runs `uv run pytest`. Each fuzz finding has a named regression test built from the seed's exact tiles and positions. |
