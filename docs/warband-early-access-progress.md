...
| W07 presentation | partial | Eight facings × four frames, dissolve deaths, arrows and stones, bursts, three themes; construction sites rise through stages and battered buildings smoke, second track `vigil` alternates with `march` (`22423d1`, frames inspected). 17 sounds, a checklist in `tests/warband/test_sound.py`. Missing: animated water, burning damage, a capture. |
...
| W10 stability, performance | partial | Fuzz across the increments: 84 AI matches and 58 monkey runs, 2 movement deadlocks found and fixed with regression tests (head-on separation `feb8046`; corner steering `bbdbd53`). Profile of a 150-unit late-game battle on the real backend (`22423d1`, scratchpad `perf.py`): p50 23.6 ms, p95 126 ms, max 751 ms with 116 units alive, heap +3 MB over 500 frames. **The 16 ms p95 is not met**; the remaining cost is the pyglet sprite migration between y-sort groups and Python-side A*. Still to run: 300-match fuzz, 100 × 1,000-step monkey, 30-minute soak (`tools/soak_warband.py`). |
