...
- Playable slice: 4 units, 4 buildings, one AI, procedural summer maps, fog,
  ...
  Warband), `tools/fuzz.py` 20 matches + 12 monkey runs clean,
  `tools/verify.py` seven real-input steps with inspected frames.
...
|---|---|---|
| W01 skirmish, difficulties, sizes | **evidence in place** | Three difficulties (`d4465fe`); 2–4 players on three sizes (`c8d0aec`); real-input match steps in `tools/verify.py` (pass on the final code). `tools/ai_report.py --seeds 6 --decide 20 --ladder 4` on the final code (2026-09-06, Normal toned down so a plain opening has a chance): the scripted opening beat Easy 5/6, Normal 2/6, Hard 1/6; Normal-vs-Normal on Medium decided 19/20 within 20 minutes, mean 11.5 min. **2026-09-19 (WB-014):** the gate had slipped: since soldiers fight soldiers first (`b26e016`, 2026-09-15, found by bisecting main), Easy's wave of ten at four and a half minutes killed the scripted opening's first soldiers and then its peasants, and the script beat Easy 5 times in 32. Easy now sends no wave before minute eight (`Profile.first_attack`): `tools/ai_report.py --seeds 16 --decide 16` → the script beat Easy 10/16, Medium, Hard and Master 0/16; Medium against Medium decided 16/16 within 20 minutes, mean 7.8 min. |
| W02 content floor | **evidence in place** | 7 units, 9 buildings + mine, 9 upgrades, tech chain (`d4465fe`): every entry reachable through the card, rendered (inspected sheet and base frame), in the F2 codex, saved, and covered by `tests/warband/test_content.py` and `test_scene.py`. Balance table in `warband/sim/rules.py`. Peasants repair damaged buildings for half the price pro rata (`22afdc9`; model, scene and AI tests). Compositions in play: the ladder shows Hard's siege, clerics and harass beating Normal 5/8, Normal's tech beating Easy 7/8. |
| W03 maps | **evidence in place** | Three themes, three sizes, 2–4 players, seed choice (`c8d0aec`); `tests/warband/test_maps.py` checks 40 seeds × 3 sizes × 3 themes for open ground, mine and wood distance, connectivity, expansions and terrain mix; winter and wasteland frames inspected. `tools/map_report.py --seeds 100` (900 maps, 2026-09-05): every base has 129–149 open tiles of 169 around its hall, its mine 5 tiles away, wood 5–7 tiles away, two expansion mines, no disconnected map, no base without wood; wasteland has fewer trees and less water than summer. 2026-09-15: five layouts (Plains, Forest, Crossings, Klondike, Bastion, or Any drawn from the seed) on a symmetric skeleton with naturals and contested thirds, sizes 48×40, 64×48, 80×64; `test_maps.py` now runs 40 seeds × 3 sizes × 5 layouts; theme is a palette only; design and evidence in `warband-maps.md`. |
...
| W05 controls | **evidence in place** | Patrol, double-click / ctrl-click type selection, Ctrl+A, bookmarks, idle button (`feb8046`); the army hotkey answers to Cmd+A as well, beside an Army button that counts the soldiers it takes; shift-queue, attack-move, hold, rally, groups, tooltips with reasons (`7cea876`); scene tests per control; hint strip per state. Three switchable control schemes, endless training, Shift-placing and the planner's spot ([controls](controls.md), `tests/warband/test_controls.py`). |
...
| W10 stability, performance | **frame gate missed** since the melee, painted-art and bars work; stability evidence in place | Fuzz across the increments: 120 AI matches and 182 monkey runs; five movement deadlocks found and fixed with regression tests (head-on separation `feb8046`; corner steering `bbdbd53`; a walk held short of its spot by a pinned crowd `7c7f3cd`; a crowd shoving units through a tree wall into an isolated clearing, and a detour that kept aiming at a spot across a blocked corner, `1b46664` — seeds 2203 and 2277 of the 300-match run, each replayed clean on the run's own code with the fix swapped in). Monkey: `tools/fuzz.py --games 0 --monkey 100 --steps 1000 --seed 1000` → 100 played, 0 failed. Frame times from `tools/perf.py` (150 units of six types between twelve farms, real backend, images pre-rendered as after the opening's warm-up): final code, 2026-09-06, load average 5 from other work: `720 frames: p50 5.2 ms, p95 10.9 ms, max 29.8 ms; last 120 frames: p50 4.7 ms, p95 9.5 ms, max 19.2 ms` (the 2026-09-05 run on the same Mac gave p95 13.1 ms; a run alongside two other CPU-bound processes gave 17–20 ms). The gate is met on an idle reference Mac. Cuts that got there: GL error checking off, pooled `draw_image` sprites, appearance setters that skip unchanged values, `Y_SORT_STEP` groups, grid A*, direct steering, cached panel geometry. The earlier 126 ms p95 figure was measured under `tracemalloc`, which the tool avoids. 300-match run (`--seed 2000`, pre-fix code): 300 played, 2 failed (the two clearing deadlocks above), 229 decided, 37 eliminations, 32 undecided. Soak (`tools/soak.py --minutes 30`, real backend, the human side played by a brain, on a loaded machine): 4 matches, no crash, heap growth −0.2 MB, p50 9–14 ms, p95 16–26 ms, worst frame 384 ms at a match start; frames saved per match and inspected. Final 300-match run on the current code (`--seed 3000`, after the clearing fixes, repair and the merges): 300 played, 0 failed, 216 decided, 39 eliminations, 45 undecided. Queued on the final code: the AI report and an idle-machine perf run. **2026-09-18 (WB-009):** on the same Mac the reference battle is late p50 9.2–10.8 / p95 16.7–18.4 ms, and 16.6–18.1 at p95 in the four-player, pan-zoom, deaths and restart scenarios: the gate is missed by one to two milliseconds since the melee, painted-art and health-bar work. Traced: the engine's `end_frame` 4.5 ms of a 9.3 ms median frame, the world step's path planning (45 % of a 3.5 ms step; up to 28 ms in a burst), and 26–41 ms collector pauses every two seconds from the renderer's unbounded groups and domains. Fixed here: the match is frozen out of the collector after warm-up. Filed: S2D-016, S2D-017; the planning share is WB-024. **2026-09-19 (WB-024):** that planning share was the reference battle's own: its seed had come to draw a wooded layout, 72 of its 150 soldiers were placed in trees, and 41 were still in them 300 steps on, planning a way out every 0.6 s. On the battlefield cleared to grass (`tools/step_bench.py`), pathfinding is 0.3 % of frame time (33 ms over 720 frames, where the old battle spent 411 ms) and the step 2.25 ms, 5.4 %; the source's step p95 is 3.6 ms. The frame p95 is 18–20 ms on the same Mac under other sessions' load, set by the gen-2 collector pauses (about 25 ms each) and the renderer's end of frame: S2D-016 and S2D-017. WB-024 closed on this. |
...
| W14 release pack | **evidence in place** | [Release pack](warband-release.md): store description, controls, known issues, credits and provenance (art/audio generated in-repo; Nunito under OFL). Updated 2026-09-08 with the published preview, exact source/artifact hashes, matching macOS evidence and platform limitations. Original controls/content audit: 2026-09-05 against `warband/ui/scene.py` and `warband/sim/rules.py`. |
...
- 2026-09-05/06: `d270bbb` the build menu offers all nine buildings (the
  ...
  setters, `tools/perf.py`); `32e05a8` moving water; `029a61b`
...
uv run python -u tools/fuzz.py --games 300 --monkey 0 --seed 3000   # W10 matches
uv run python -u tools/fuzz.py --games 0 --monkey 100 --steps 1000 --seed 1000  # W10 monkey
uv run python tools/soak.py --minutes 30 --out /tmp/warband_soak    # W10 soak
uv run python tools/perf.py                                # W10 frame times
uv run python tools/verify.py /tmp/warband_verify          # W01/W05 real input, frames to look at
