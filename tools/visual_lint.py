...
on the mock backend (with real glyph metrics) and reports what each one drew.
...
def run_screen(name: str, resolution: tuple[int, int], evidence: Path | None) -> list[lint.Finding]:
    ...
    with tempfile.TemporaryDirectory(prefix="warband-lint-") as scratch:
        game = Game("Warband lint", backend="mock", resolution=resolution, theme=build_theme(), save_dir=Path(scratch) / "saves")
        try:
        ...
        if evidence is not None:
            game = Game("Warband lint", backend="pyglet", resolution=resolution, visible=False, theme=build_theme(), save_dir=Path(scratch) / "saves")
            try:
                fonts.load(game)
                SCREENS[name](game)
...
def main() -> None:
    ...
    if not args.no_images:
        game = Game("Warband lint", backend="mock", resolution=RESOLUTIONS[0], theme=build_theme())
        store = lint.ImageStore(game)
        lint.register_everything(game)
        findings = lint.lint_images(game, store)
        game._teardown()
        print(f"images: {len(game.assets._images)} registered, {len(findings)} findings")
