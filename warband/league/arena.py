...
here touches saga2d — it is the model, the agents and arithmetic.
    ...
    from warband.league.arena import MatchSpec, play, rate
...
class Agent:
    ...
    :class:`warband.brains.ai.Brain` and :class:`warband.brains.pro_ai.ProBrain` satisfy this
...
from warband.league.archetypes import ARCHETYPES  # noqa: E402 - the balance league's postures play under their names too
...
class _Balance:
    ...
    ``.buildings`` (derived from :mod:`warband.sim.rules` at import time) while the
...
class MatchResult:
    ...
    tallies: tuple[PlayerTally, ...] = ()  # what each player bought, lost and killed; see :mod:`warband.league.telemetry`
