...
def register_everything(game: Game, *, players: tuple[int, ...] = (0, 1)) -> None:
    ...
    for race in Race:
        list(textures.warm_units(game, [0], [race]))
...
def use_real_text_metrics(game: Game) -> None:
    """Measure text on the mock backend with the bundled Nunito faces instead of a fixed
    advance per character, so layout findings mean what they would on screen (pyglet's
    numbers differ from PIL's by a few per cent at most)."""
