...
Nothing here plays a match; it reads :class:`~warband.arena.MatchResult`
tallies (see :mod:`warband.telemetry`).
"""
...
def save(results: Iterable[MatchResult], path: Path) -> None:
    """One JSON line per match: the spec, the outcome and every tally."""
    ...
    with path.open("w") as out:
        for result in results:
            ...
            row["tallies"] = [{**{k: (dict(v) if isinstance(v, Counter) else v) for k, v in vars(t).items()
                                  if k != "timeline"}, "timeline": [asdict(s) for s in t.timeline]}
                              for t in result.tallies]
            out.write(json.dumps(row) + "\n")


def load(path: Path) -> list[MatchResult]:
    results = []
    with path.open() as source:
        for line in source:
            row = json.loads(line)
            spec = row.pop("spec")
            spec["agents"] = tuple(spec["agents"])
            if spec["races"] is not None:
                spec["races"] = tuple(spec["races"])
            tallies = tuple(_tally(t) for t in row.pop("tallies"))
            results.append(MatchResult(spec=MatchSpec(**spec), placements=tuple(row.pop("placements")),
                                       tallies=tallies, **row))
    return results


def _tally(row: dict) -> PlayerTally:
    timeline = [Sample(**s) for s in row.pop("timeline")]
    ...
    return PlayerTally(**rest, **counters, timeline=timeline)
